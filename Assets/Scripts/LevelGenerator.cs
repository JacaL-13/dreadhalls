...
public class LevelGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        Vector3[] holes = new Vector3[holeCount];

        for (int i = 0; i < holeCount; i++)
        {
            int holeX = Random.Range(1, mazeSize - 1);
            int holeZ = Random.Range(1, mazeSize - 1);

            holes[i] = new Vector3(holeX, 1, holeZ);

            // Debug.Log("Hole " + i + " at " + holes[i]);
        }
        ...
        for (int z = 0; z < mazeSize; z++)
        {
            for (int x = 0; x < mazeSize; x++)
            {
                ...
                // check current position is a hole. If the character isn't placed, always place a floor
                if (
                    System.Array.Exists(holes, element => element == new Vector3(x, 1, z)) == false
                    || !characterPlaced
                )
                {
